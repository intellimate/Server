...
import org.intellimate.server.proto.UserAuthentication;
...
public class Authentication {
     ...
     * @return if valid an access jwt-token and the ID
     */
    public UserAuthentication login(String email, String password) {
        ...
        if (valid) {
            String token = jwtHelper.generateUserAccessJWT(user.getIdUser());
            return UserAuthentication.newBuilder()
                    .setToken(token)
